...
#    freq = config['model'].get('fcst_hour_int', 12)
    ...
    for fhr in range(0, int(fmax)+int(freq), int(freq)):
       ...
       if not os.path.isfile('f{0}_enfo.grb2'.format(hhh)) and os.path.isfile('{0}/{1}/f{2}_enfo.grb2'.format(config['locations']['model_dir'],datea,hhh)):
          ...
          os.symlink('{0}/{1}/f{2}_cont.grb2'.format(config['locations']['model_dir'],datea,hhh),'f{0}_cont.grb2'.format(hhh))
...
def download_grib_file(args):
    ...
    varstr = ':u:|:v:|:t:|:q:|:10[uv]:|:msl:|:tcw:|:tp:'

    #  Fix for heights including surface
    zlevs = []
    for lev in [10, 50, 100, 150, 200, 250, 300, 400, 500, 600, 700, 850, 925, 1000]:
      zlevs.append('|z:{0}:'.format(lev))
    varstr = varstr + ''.join(zlevs)       

    #  Read the ECMWF members
    H = Herbie(initstr, model='aifs', product='enfo', fxx=fhr)
    ...
    H = Herbie(initstr, model='aifs', product='enfo', fxx=fhr, get_control=True)
    H.LOCALFILE = "{0}/f{1}_cont.grb2".format(config['locations']['work_dir'],fhrt)
    file1 = H.download(varstr)
    os.rename(file1, 'f{0}_cont.grb2'.format(fhrt))
...
def stage_atcf_files(datea, bbnnyyyy, config):
    ...
    if not os.path.isfile('{0}/atcf_{1}.dat'.format(config['locations']['work_dir'],'%0.2i' % nens)):
       ...
       if int(bbnnyyyy[2:4]) < 50:
          ...
          while not os.path.exists(src) and len(glob.glob('{0}/*{1}0000*{2}*.dat'.format(config['locations']['atcf_dir'],datea,config['storm'][0:-3].upper()))) < 1:
             time.sleep(20.5)

          #  Use the initialization time file, or storm file as second priority
          while True:

             alt_src = glob.glob('{0}/*{1}0000*{2}*.dat'.format(config['locations']['atcf_dir'],datea,config['storm'][0:-3].upper()))
             if len(alt_src) > 0:
                src = max(alt_src, key=os.path.getsize)
                break
             elif len(os.popen('sed -ne /{0}/p {1} | sed -ne /AI/p'.format(datea,src)).read()) > 0:
                break
          ...
          for n in range(nens + 1):
             ...
             fo = open(file_name,"w")
             fo.write(os.popen('sed -ne /{0}/p {1} | sed -ne /AI{2}/p'.format(datea,src,nn)).read())
             fo.close()
       ...
       #  Otherwise, go through individual tracker files to find INVEST areas.  Use GEFS as guess 
          ...
          for n in range(0,nens+1):
             ecmid.append('AI{0}'.format('%0.2i' % n))
...
class ReadGribFiles:
    ...
    def __init__(self, datea, fhr, config):
        ...
        try:
           ...
           #  Water vapor is unique, so it needs to be read separately
           ds = xr.open_dataset(file_name, engine='cfgrib', backend_kwargs={'filter_by_keys': {'typeOfLevel': 'isobaricInhPa', "shortName": "q"}})
           for tt in ds:
        ...
        file_name = "{0}/f{1}_cont.grb2".format(config['locations']['work_dir'], '%0.3i' % fhr)
    ...
    def create_ens_array(self, varname, nens, vdict):
       ...
       if 'units' in self.grib_dict[vname].attrs:
         attrlist['units'] = self.grib_dict[vname].attrs['units']
       elif 'units' in vdict:
